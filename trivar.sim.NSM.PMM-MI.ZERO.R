# Simulation from West and Little (2013), using PMM-MI approach based on a normal selection model

trivar.sim.nsm.pmm.mi <- function(nsamps,sampsize,rrate,beta,rho,myseed,m=100)
{
   require(mnormt)
   ...
   results <- matrix(0,nrow=nsamps,ncol=11)
   set.seed(myseed)
   ...
   for (s in 1:nsamps)
   {
      ...
      results[s,1] <- n0/sampsize # response rate
      ...
      results[s,4] <- (results[s,2] - 1)^2 
      results[s,5] <- (results[s,3] - 10)^2 
      ...
      if (x2mean.ll <= 1 && x2mean.ul > 1) results[s,6] <- 1
      else results[s,6] <- 0
      ...
      results[s,10] <- (1 + 1/m) * var(x2meanimpvec) / mi.var.x2 # FMI for X2 mean
      results[s,11] <- (1 + 1/m) * var(x3meanimpvec) / mi.var.x3 # FMI for X3 mean

      cat("Sample:"," ",s,"\n")

   } # end of for loop for simulations

   cat("Mean response rate is:"," ",mean(results[,1]),"\n")

   cat("True mean of X2 is:"," ",1,"\n")
   cat("\n")

   cat("Mean of PMM-MI estimates of X2 mean for respondents is:"," ",mean(results[,2]),"\n")
   cat("RelBias of PMM-MI estimates of X2 mean is:"," ",((mean(results[,2]) - 1)/1)*100,"\n")
   ...
   cat("FMI for mean of X2 is:"," ",mean(results[,10]),"\n")
   ...
   cat("RelBias of PMM-MI estimates of X3 mean is:"," ",((mean(results[,3]) - 10)/10)*100,"\n")
   ...
   cat("FMI for mean of X3 is:"," ",mean(results[,11]),"\n")
...
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.5,beta=2,rho=0.9,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.5,beta=1,rho=0.9,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.5,beta=0,rho=0.9,myseed=41279,m=100)

tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.5,beta=2,rho=0.6,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.5,beta=1,rho=0.6,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.5,beta=0,rho=0.6,myseed=41279,m=100)

tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.268945,beta=2,rho=0.9,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.268945,beta=1,rho=0.9,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.268945,beta=0,rho=0.9,myseed=41279,m=100)

tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.268945,beta=2,rho=0.6,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.268945,beta=1,rho=0.6,myseed=41279,m=100)
tres <- trivar.sim.nsm.pmm.mi(nsamps=1000,sampsize=1000,rrate=0.268945,beta=0,rho=0.6,myseed=41279,m=100)
