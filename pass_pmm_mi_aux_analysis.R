...
# aux.data has n rows and at least 7 columns (last three are strata, PSUs, weights),
# response has n indicators of response, m is the number of multiples, and k is number of additional auxiliary variables.
# last six columns are X1, X2, X3, strata, PSU, and weight (order matters).
# first k columns are additional auxiliary variables (EXCLUDING the column of 1s for the intercept).

aux.mi.analysis <- function(aux.data, response, m=100, k)
{
       # load survey package
       ...
       b1c.c.0.draws <- matrix(0,nrow=k+1,ncol=m)
       s11.c.1.draws <- numeric(m)
       b1c.c.1.draws <- matrix(0,nrow=k+1,ncol=m)
       ...
       b2c.c.1.draws <- matrix(0,nrow=k+1,ncol=m)
       b3c.c.1.draws <- matrix(0,nrow=k+1,ncol=m)
       ...
       b2c.1c.1.draws <- matrix(0,nrow=k+1,ncol=m)
       b21.1c.1.draws <- numeric(m)
       s22.1c.1.draws <- numeric(m)
       b3c.21c.1.draws <- matrix(0,nrow=k+1,ncol=m)
       b31.21c.1.draws <- numeric(m)
       b32.21c.1.draws <- numeric(m)
       s33.21c.1.draws <- numeric(m)

       # Regress X1 on C for complete cases

       xnam <- paste("trivar.0[,", 1:k, "]", sep="")
       fmla <- as.formula(paste("trivar.0[,k+1] ~ ", paste(xnam, collapse= "+")))
       #fit1c.0 <- lm(trivar.0[,k+1] ~ trivar.0[,1:k])
       fit1c.0 <- lm(fmla)
       b1c.c.0 <- summary(fit1c.0)$coef[,1]
       sigma11.c.0 <- summary(fit1c.0)$sigma^2
       ssc1.c.0 <- t(model.matrix(fit1c.0)) %*% model.matrix(fit1c.0)

       # Regress X1 on C for incomplete cases

       xnam <- paste("trivar.1[,", 1:k, "]", sep="")
       fmla <- as.formula(paste("trivar.1[,k+1] ~ ", paste(xnam, collapse= "+")))
       #fit1c.1 <- lm(trivar.1[,k+1] ~ trivar.1[,1:k])
       fit1c.1 <- lm(fmla)
       b1c.c.1 <- summary(fit1c.1)$coef[,1]
       sigma11.c.1 <- summary(fit1c.1)$sigma^2
       ssc1.c.1 <- t(model.matrix(fit1c.1)) %*% model.matrix(fit1c.1)

       # Regress X1 on X2 and C for complete cases

       xnam <- paste("trivar.0[,", 1:k, "]", sep="")
       fmla <- as.formula(paste("trivar.0[,k+1] ~ ", paste(xnam, collapse= "+"), "+ trivar.0[,k+2]"))
       #fit12c.0 <- lm(trivar.0[,k+1] ~ trivar.0[,c(1:k,k+2)])
       fit12c.0 <- lm(fmla)
       b12.2c <- summary(fit12c.0)$coef[k+2,1]
       sigma11.2c.0 <- summary(fit12c.0)$sigma^2

       # Regress X3 on X2 and C for complete cases

       xnam <- paste("trivar.0[,", 1:k, "]", sep="")
       fmla <- as.formula(paste("trivar.0[,k+3] ~ ", paste(xnam, collapse= "+"), "+ trivar.0[,k+2]"))
       #fit32c.0 <- lm(trivar.0[,k+3] ~ trivar.0[,c(1:k,k+2)])
       fit32c.0 <- lm(fmla)
       b32.2c <- summary(fit32c.0)$coef[k+2,1]
       sigma33.2c.0 <- summary(fit32c.0)$sigma^2

       # Regress X2 on C for complete cases
 
       xnam <- paste("trivar.0[,", 1:k, "]", sep="")
       fmla <- as.formula(paste("trivar.0[,k+2] ~ ", paste(xnam, collapse= "+")))
       #fit2c.0 <- lm(trivar.0[,k+2] ~ trivar.0[,1:k])
       fit2c.0 <- lm(fmla)
       b2c.c.0 <- summary(fit2c.0)$coef[,1]
       sigma22.c.0 <- summary(fit2c.0)$sigma^2

       # Regress X3 on C for complete cases
 
       xnam <- paste("trivar.0[,", 1:k, "]", sep="")
       fmla <- as.formula(paste("trivar.0[,k+3] ~ ", paste(xnam, collapse= "+")))
       #fit3c.0 <- lm(trivar.0[,k+3] ~ trivar.0[,1:k])
       fit3c.0 <- lm(fmla)
       ...
       mu1.0 <- mean(trivar.0[,k+1])
       mu2.0 <- mean(trivar.0[,k+2])
       mu3.0 <- mean(trivar.0[,k+3])
       ...
       for (d in 1:m) 
       {
          ...
          s11.c.0.draws[d] <- n0*sigma11.c.0 / rchisq(1,n0-k-1)
          b1c.c.0.draws[,d] <- rmnorm(1, b1c.c.0, solve(ssc1.c.0)*s11.c.0.draws[d])
          s11.c.1.draws[d] <- n1*sigma11.c.1 / rchisq(1,n1-k-1)
          b1c.c.1.draws[,d] <- rmnorm(1, b1c.c.1, solve(ssc1.c.1)*s11.c.1.draws[d])
          s11.2c.draws[d] <- n0*sigma11.2c.0 / rchisq(1,n0-k-1)
          s33.2c.draws[d] <- n0*sigma33.2c.0 / rchisq(1,n0-k-1)
	  ...
	  s33.21c.1.draws[d] <- (s33.c.1.draws[d] - s13.c.1.draws[d]^2 / s11.c.1.draws[d]) -
          ...
          trivar.1[,k+2] <- rnorm(length(trivar.1[,k+2]), mean = model.matrix(fit1c.1) %*% b2c.1c.1.draws[,d] + b21.1c.1.draws[d] * trivar.1[,k+1], 
	  	sd = sqrt(s22.1c.1.draws[d]))

	  trivar.1[,k+3] <- rnorm(length(trivar.1[,k+3]), mean = model.matrix(fit1c.1) %*% b3c.21c.1.draws[,d] + b31.21c.1.draws[d] * trivar.1[,k+1] +
	  	b32.21c.1.draws[d] * trivar.1[,k+2], sd = sqrt(s33.21c.1.draws[d])) 
          ...
          trivar.imp.data2 <- trivar.imp.data[!is.na(trivar.imp.data[,k+6]) &  !is.na(trivar.imp.data[,k+5]) & !is.na(trivar.imp.data[,k+4]),]
          svyd <- svydesign(strata=~trivar.imp.data2[,k+4], id=~trivar.imp.data2[,k+5], weights=~trivar.imp.data2[,k+6] , data=trivar.imp.data2, nest=T, na.rm=T)

          # save estimate and variance for MI analysis

          x2meanimpvec[d] <- coef(svymean(~trivar.imp.data2[,k+2],svyd,na.rm=T))
          x3meanimpvec[d] <- coef(svymean(~trivar.imp.data2[,k+3],svyd,na.rm=T))
          x2varimpvec[d] <- SE(svymean(~trivar.imp.data2[,k+2],svyd,na.rm=T))^2
          x3varimpvec[d] <- SE(svymean(~trivar.imp.data2[,k+3],svyd,na.rm=T))^2
       }
...
# Example using passdata.csv file
trivar <- read.csv("J:\\Dissertation\\Paper 3\\passdata.csv",h=T)
trivar$proxy <- trivar[,1] / 10000
trivar2 <- data.frame(cbind(trivar[,5],trivar[,1],trivar[,3],trivar[,4],trivar[,7],trivar[,6],trivar[,5]))
resp <- trivar[,2]

aux.mi.analysis(trivar2,resp,m=100,k=1)
